...
def ffmpeg_process(youtube: YouTube, resolution: str, target: Optional[str] = None) -> None:
    ...
    if resolution in (None, 'best'):
        highest_quality_stream = youtube.streams.filter(
            progressive=False).order_by("resolution").last()
        mp4_stream = youtube.streams.filter(
            progressive=False, subtype="mp4").order_by("resolution").last()
        ...
        video_stream = youtube.streams.filter(
            progressive=False, resolution=resolution).first()
    ...
    audio_stream = youtube.streams.filter(
        progressive=False).order_by("abr").last()

    video_file_name = _unique_name(youtube.title, "mp4", "video", target)
    audio_file_name = _unique_name(youtube.title, "mp4", "audio", target)

    video_path = video_stream.get_file_path(
        filename=video_file_name, output_path=target)
    audio_path = audio_stream.get_file_path(
        filename=audio_file_name, output_path=target)
    ...
    command = ["ffmpeg", "-i", video_path, "-i", audio_path, "-c:v", "copy",
               "-c:a", "aac", "-strict", "experimental", f"{target}/{youtube.title}.mp4"]
...
def download_audio(youtube: YouTube,
    ...
    stream = youtube.streams.filter(
        progressive=False, subtype=filetype).order_by("abr").last()
    if stream is None:
...
def _parse_args(parser: argparse.ArgumentParser, args: Optional[List] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--itag", type=int,
    ...
    parser.add_argument("-l", "--list", action="store_true",
                        help="The list option causes pytubefix cli to return a list of streams available to download")
    parser.add_argument("--oauth", action="store_true", help="use oauth token")
    parser.add_argument("-v", "--verbose", action="store_true",
                        dest="verbose", help="Set logger output to verbose output.")
    parser.add_argument("--logfile", action="store",
                        help="logging debug and error messages into a log file")
    parser.add_argument("--build-playback-report",
                        action="store_true", help="Save the html and js to disk")
    parser.add_argument("-c", "--caption-code", type=str,
                        help="Download srt captions for given language code. Prints available language codes if no argument given")
    parser.add_argument('-lc', '--list-captions', action='store_true',
                        help="List available caption codes for a video")
    parser.add_argument(
        "-t", "--target", help="The output directory for the downloaded stream. Default is current working directory")
    parser.add_argument("-a", "--audio", const="mp4", nargs="?",
                        help="Download the audio for a given URL at the highest bitrate available. Defaults to mp4 format if none is specified")
    parser.add_argument("-f", "--ffmpeg", const="best", nargs="?",
                        help="Downloads the audio and video stream for resolution provided. If no resolution is provided, downloads the best resolution. Runs the command line program ffmpeg to combine the audio and video")
...
def _perform_args_on_youtube(youtube: YouTube, args: argparse.Namespace) -> None:
    if len(sys.argv) == 2:
        download_highest_resolution_progressive(
            youtube=youtube, target=args.target)
    ...
    if args.ffmpeg:
        ffmpeg_process(youtube=youtube,
                       resolution=args.resolution, target=args.target)
...
def main():
    ...
    if args.verbose:
        logger.debug('Pytubefix version: %s', __version__)
