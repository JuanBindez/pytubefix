...
class Cipher:
    def __init__(self, js: str, js_url: str):
        ...
        self._nsig_param_val = self._extract_nsig_param_val(js)
        ...
        try:
            if self._nsig_param_val:
                for param in self._nsig_param_val:
                    try:
                        return run_js_interpreter(self.js, self.nsig_function_name, [param, n])
                    except:
                        continue
            else:
    ...
    def get_sig_function_name(self, js: str, js_url: str) -> str:
        ...
        function_patterns = [
            ...
            r'\b(?P<var>[a-zA-Z0-9_$]+)&&\((?P=var)=(?P<sig>[a-zA-Z0-9_$]{2,})\((?:(?P<value>\d+),decodeURIComponent|decodeURIComponent)\((?P=var)\)\)',
            # Old patterns
        ...
        for pattern in function_patterns:
            ...
            if function_match:
                ...
                self._sig_param_val = int(function_match.group('value'))
                value = function_match.group('value')
                if value:
                    self._sig_param_val = int(value)
                return sig
    ...
    @staticmethod
    def get_nsig_function_name(js: str, js_url: str):
    ...
    def _extract_nsig_param_val(self, code) -> list:
        pattern = re.compile(
            rf'(?<![A-Za-z0-9_$\.])' 
            rf'(?P<func>{re.escape(self.nsig_function_name)})\s*'
            r'\[\w\[\d\]\]'
            r'\(\s*'
            r'(?P<arg1>[A-Za-z0-9_$]+)'
